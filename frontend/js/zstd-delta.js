 ...
 *
 * Two decoders, one surface. Facebook's zstd (BSD licence) compiled to
...
// ----------------------------------------------------------------- fzstd --
// fzstd's lib/index.js, kept whole inside this function. Its frame and
// block readers are handed out so the base page can be seeded as the
// decoder's window: a delta's back-references reach into the base exactly
// as they would into a raw-content dictionary.
var ApFzstd = (function () {
  var exports = {};
"use strict";
...
var ApZstd = (function () {
  'use strict';
  var ready = null;
  var mode = null;   // 'wasm' once the WebAssembly decoder is up, else 'js'

  // The glue logs a module that will not compile and then never starts,
  // so bytes are checked first and the start is given a deadline.
  function initWasm(source) {
    return new Promise(function (resolve, reject) {
      if (typeof WebAssembly === 'undefined') {
        reject(new Error('WebAssembly is unavailable')); return;
      }
      if (source && typeof source !== 'string' &&
          !WebAssembly.validate(new Uint8Array(source))) {
        reject(new Error('not a WebAssembly module')); return;
      }
      var timer = setTimeout(function () {
        reject(new Error('the WebAssembly decoder did not start'));
      }, 10000);
      Module['onRuntimeInitialized'] = function () { clearTimeout(timer); resolve(); };
      Module['onAbort'] = function (why) {
        clearTimeout(timer); reject(new Error('zstd failed to load: ' + why));
      };
      try { Module['init'](source); } catch (err) { clearTimeout(timer); reject(err); }
    });
  }

  /** Load the decoder once: the .wasm from a URL or an ArrayBuffer, or with
   * null the JavaScript decoder. A wasm that will not load falls back to
   * JavaScript rather than failing. Resolves with the mode in use. */
  function init(source) {
    if (!ready) {
      ready = (source == null ? Promise.reject(new Error('no wasm asked for')) : initWasm(source))
        .then(function () { mode = 'wasm'; return mode; },
              function (err) {
                if (source != null) {
                  console.warn('[zstd] WebAssembly decoder unavailable, using JavaScript:',
                               err && err.message);
                }
                mode = 'js';
                return mode;
  ...
  /** The page that `delta` encodes against `base`, in JavaScript. */
  function patchJs(delta, base) {
    var head = ApFzstd.rzfh(delta, 1);
    if (typeof head !== 'object' || !(head.u > 0)) {
      throw new Error('zstd delta carries no content size');
    }
    var out = new Uint8Array(base.byteLength + head.u);
    out.set(base, 0);
    var st = ApFzstd.rzfh(delta, out);
    st.y = base.byteLength;
    st.e = base.byteLength;
    while (!st.l) {
      if (!ApFzstd.rzb(delta, st, out)) { throw new Error('zstd delta did not apply'); }
      st.e = st.y;
    }
    if (st.y - base.byteLength !== head.u) {
      throw new Error('zstd delta rebuilt the wrong size');
    }
    return out.slice(base.byteLength, st.y);
  }

  /** The page that `delta` encodes against `base`; both Uint8Array. */
  function patch(delta, base) {
    if (mode === 'js') { return patchJs(delta, base); }
    if (mode !== 'wasm') { throw new Error('zstd decoder not initialised'); }
  ...
  return { init: init, patch: patch, mode: function () { return mode; } };
})();
