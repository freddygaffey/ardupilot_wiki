...
/*
 * Analytics, after the page is done with itself.
 *
 * This used to be declared in every wiki's conf.py as an html_js_files entry.
 * Deferred and served from cache it still cost 14 ms during the load of every
 * page, on a page whose own document took 4 ms, which made a visitor counter
 * one of the more expensive things on the site.
 *
 * Loading it after the load event counts the same pageview - the script reads
 * location when it initialises, and that does not change - and binds the same
 * outbound-link handlers a fraction of a second later than before. A reader
 * would have to click an external link within that window to lose an event.
 *
 * Kept here rather than in eleven copies of conf.py, and kept honest: if this
 * ever needs to go back to being blocking, put it back in conf.py rather than
 * making this earlier.
 */
(function () {
  'use strict';

  var SRC = 'https://plausible.ardupilot.org/js/script.outbound-links.js';
  ...
  function load() {
    ...
    s.src = SRC;
    s.defer = true;
    s.setAttribute('data-domain', 'ardupilot.org');
  ...
  else { window.addEventListener('load', whenDone, { once: true }); }
})();
