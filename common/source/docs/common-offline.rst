...
.. raw:: html
   ...
   <!-- The tool: markup and styling together, behaviour in
        /js/offline-page.js and /js/offline-export.js.

        The style is INLINE, deliberately, against the usual advice.

        It used to be a <link> to /css/offline.css emitted from layout.html.
        That put a request on all 3,958 pages of the wiki for a panel that
        exists on one, and, worse, the file belongs to frontend/ and so is in
        no wiki archive. A reader who downloaded a wiki and went offline got
        this page unstyled: the one screen whose whole job is to work without a
        connection. Inline, the style cannot be missing while the markup is
        present, because they are the same bytes.
    ...
    *
    * Inlined into the page rather than linked, so that a reader who has this
    * markup necessarily has its style too. See the comment above the <style>.
    */
